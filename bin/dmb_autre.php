...
fdfldp.field_label_du_produit_value as titre,
fdfdpa.field_description_produit_autre_value as description,
...
LEFT JOIN ' . $db_old . '.field_data_field_ville_hotel fdfvh ON fdfvh.entity_id = fdfap.field_autre_produit_value
LEFT JOIN ' . $db_old . '.field_data_field_label_du_produit fdfldp ON fdfldp.entity_id = fdfap.field_autre_produit_value
LEFT JOIN ' . $db_old . '.field_data_field_description_produit_autre fdfdpa ON fdfdpa.entity_id = fdfap.field_autre_produit_value
...
foreach ($dbh->query($query, PDO::FETCH_ASSOC) as $row) {
  ...
  $query = '
  REPLACE INTO ' . $db_new . '.autre_product
  (id,currency,group_type,label, `name`,ville,titre,description,nom_en_latin,nom_en_cyrillique,phone,supp_info,info_utile,excel_custom_id,excel_title,excel_description)
  VALUES
  (\'' . $row["nid"] . '\',\'' . $row["currency"] . '\',\'' . $row["group_type"] . '\',\'' . $row["name"] . '\',\'' . $row["name"] . '\',' . $row["ville"] . ',\'' . $row["titre"] . '\',
  \'' . addslashes($row["description"]) . '\',\'' . $row["name_en_latin"] . '\',\'' . $row["name_en_cyrillique"] . '\',\'' . $row["phone"] . '\',\'' . $row["info_sup"] . '\',\'' . $row["extra_info"] . '\',
