...
fdfv.field_vehicule_value as vehicle,
fdfd1.field_duree_value as duree,
fdfldp.field_label_du_produit_value as extension,
...
LEFT JOIN ' . $db_old . '.field_data_field_chauffeur fdfc ON fdfc.entity_id = n.nid

LEFT JOIN ' . $db_old . '.field_data_field_ville_hotel fdfvh ON fdfvh.entity_id = fdfc.field_chauffeur_value
LEFT JOIN ' . $db_old . '.field_data_field_vehicule fdfv ON fdfv.entity_id = fdfc.field_chauffeur_value
LEFT JOIN ' . $db_old . '.field_data_field_duree fdfd1 ON fdfd1.entity_id = fdfc.field_chauffeur_value
LEFT JOIN ' . $db_old . '.field_data_field_label_du_produit fdfldp ON fdfldp.entity_id = fdfc.field_chauffeur_value
LEFT JOIN ' . $db_old . '.field_data_field_commentaires fdfc1 ON fdfc1.entity_id = fdfc.field_chauffeur_value
...
foreach ($dbh->query($query, PDO::FETCH_ASSOC) as $row) {
  ...
  $query = '
  REPLACE INTO ' . $db_new . '.chauffeur
  (id,type_de_vehicle,duree,currency,group_type,label,`name`,ville,extension,comments,nom_en_latin,nom_en_cyrillique,phone,supp_info,info_utile,excel_custom_id,excel_title,excel_description)
  VALUES
  (\'' . $row["nid"] . '\',\'' . $row["vehicle"] . '\',\'' . $row["duree"] . '\',\'' . $row["currency"] . '\',\'' . $row["group_type"] . '\',\'' . $row["name"] . '\',\'' . $row["name"] . '\',' . $row["ville"] . ',\'' . $row["extension"] . '\',
